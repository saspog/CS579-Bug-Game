    ...
    void Update()
    {
        if (this.lastSpawn >= secCheck) //time passed, random determ
        {
            ...
            if(this.rand.Next(1,101) <= (maxBugs - this.inField.Count)) {
                ...
                float x = (float)this.rand.NextDouble()*10-5;
                float y = (float)this.rand.NextDouble()*10-5;
                float z = (float)this.rand.NextDouble()*10-5;
        ...
        {
            if (!this.inField[i].GetComponent<BugBase>().getCaught()) { this.inField[i].SetActive(true); }
            else { this.inField.RemoveAt(i); }
        }
    }
}
